...
void FFT::update()
{
	...
	*/
	...
	cal_Gain_smoothed(Gain_smoothed, Gui_Global->FFT__k_smooth);
	{
...
void FFT::cal_Gain()
{
	...
	if(Gui_Global->FFT__b_Window)	_acf = acf;
	else							_acf = 1.0f;
	
	for(int i = 0; i < Gain.size(); i++){
		Gain[i] = 2 * sqrt( fft_x[i] * fft_x[i] + fft_y[i] * fft_y[i] ) / _acf * Gui_Global->FFT__SoftGain;
	}
}
